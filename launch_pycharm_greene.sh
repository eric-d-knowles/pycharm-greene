...
cleanup() {
  ...
  pkill -f "ssh .*greene-compute.* -N -f -L"  >/dev/null 2>&1 || true
...
if [[ -n "$OVERLAY_PATH" ]]; then
  PROMPT_OVERLAY="Overlay path (last: $OVERLAY_PATH): "
else
  PROMPT_OVERLAY="Overlay path: "
fi
...
if [[ -n "$CONDA_ENV_NAME" ]]; then
  PROMPT_CONDA="Conda environment name (last: $CONDA_ENV_NAME): "
else
  PROMPT_CONDA="Conda environment name: "
fi
...
while true; do
  _prompt "$PROMPT_OVERLAY" "$OVERLAY_PATH" OVERLAY_PATH
  if [[ -z "$OVERLAY_PATH" ]]; then
    printf "${RED}Overlay path is required. Please enter a value.${RST}\n" >&3
    continue
  fi
  _sanitize_printable OVERLAY_PATH
  break
done
...
while true; do
  _prompt "$PROMPT_CONDA" "$CONDA_ENV_NAME" CONDA_ENV_NAME
  if [[ -z "$CONDA_ENV_NAME" ]]; then
    printf "${RED}Conda environment name is required. Please enter a value.${RST}\n" >&3
    continue
  fi
  _sanitize_printable CONDA_ENV_NAME
  break
done
...
ssh_try 'JB="/scratch/$USER/.jb"; mkdir -p "$JB"; printf "%s\n" "'"$JOB_NAME"'" > "$JB/job_name"; printf "%s\n" "'"$CONDA_ENV_NAME"'" > "$JB/conda_env"'
...
# --- Overwrite greene-compute HostName in ~/.ssh/config ---
if [[ "$NODE" == *.* ]]; then
  FQDN="$NODE"
else
...
# Ensure greene-compute stanza exists
if ! grep -q '^Host[[:space:]]\+greene-compute' "$SSH_CONFIG"; then
  {
    echo
    echo "Host greene-compute"
...
if ! sed -i.bak -E "/^Host[[:space:]]+greene-compute$/,/^Host[[:space:]]/ s|^([[:space:]]*HostName).*|\1 $FQDN|" "$SSH_CONFIG" 2>/dev/null; then
  perl -0777 -pe '
    if(s/^Host\s+greene-compute\b.*?(?=^Host\s|\z)/$&/ms){
      s/^(\s*HostName).*/$1 '"$FQDN"'/m
    }' -i.bak -- "$SSH_CONFIG"
fi

printf "Updated ~/.ssh/config: greene-compute -> %s\n" "$FQDN"
...
# Singularity args
sing_args=( exec --overlay "$OVERLAY_PATH:rw" )
